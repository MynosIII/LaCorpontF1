...
function historyChart(data) {
  ...
  </svg></div><div class="legend">${selected.map((driver,index) => `<span><i style="background:${colors[index]}"></i>${driver.name}</span>`).join("")}</div>`;
}
...
function render(data) {
  ...
  <section aria-labelledby="history-title"><div class="section-title"><div><p class="number">01</p><h2 id="history-title">Evolución histórica</h2></div><p>Rendimiento ELO a través del tiempo</p></div>${historyChart(data)}</section>
